...
# Note: genai.Client is used for the new SDK
...
class GeminiService:
    def __init__(self):
        self._disease_cache: dict | None = None

    async def _call_gemini(self, prompt: str, as_json: bool = False) -> str | dict | list:
    ...
    async def generate_text(self, prompt: str) -> str:
        ...
        result = await self._call_gemini(prompt, as_json=False)
        return str(result)

    async def get_disease_list(self) -> list[dict]:
...
Categories: Infectious, Cardiovascular, Respiratory, Neurological, Gastrointestinal, Endocrine, Oncology, Musculoskeletal, Dermatological, Psychiatric"""
        result = await self._call_gemini(prompt, as_json=True)
        self._disease_cache = result if isinstance(result, list) else []
        return self._disease_cache

    async def get_tests_for_disease(self, disease_name: str) -> list[dict]:
...
Return 5-10 most clinically relevant tests."""
    ...
    async def get_symptoms_for_disease(self, disease_name: str) -> list[str]:
...
Return JSON: {{
  ...
  "recommendation": str
    ...
    async def generate_explainability(self, state_dict: dict) -> dict:
...
Return JSON: {{
  ...
  "uncertainty_flags": [str]
    ...
    async def generate_consultation_report(self, state_dict: dict) -> dict:
...
Return JSON: {{
  ...
  "patient_friendly_summary": str
    ...
    async def run_cusum_analysis(self, scores_list: list[float]) -> dict:
...
Return JSON: {{
  ...
  "statistical_note": str
    ...
    async def analyze_medical_image(self, image_b64: str, hint: str = "") -> dict:
...
Keep it simple, hopeful, and under 200 words. No medical jargon."""
        return await self._call_gemini(prompt, as_json=False)

    async def generate_patient_friendly_report(self, report_content: str, patient_name: str) -> str:
...
Return JSON: {{"alert_message": str, "risk_level": "low|medium|high", "doctor_notification": bool, "patient_guidance": str}}"""
    ...
    async def patient_chatbot(self, message: str, patient_context: dict) -> str:
